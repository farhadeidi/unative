"use client";
import {
  defaultOptions,
  defaultVariant,
  IconProps,
  IconVariants,
} from "@unative/icons/icon-options";
import { useContext } from "react";
import { cn } from "unative/lib/utils";
import { G, Path, Svg } from "unative/ui/svg";
import { TextClassContext } from "unative/ui/text";

const AddCircleBold = ({ size = 24, ...props }: IconProps) => {
  delete props.alterOpacity;

  return (
    <Svg width={size} height={size} viewBox="0 0 24 24" fill="none" {...props}>
      <Path
        d="M12 2C6.49 2 2 6.49 2 12C2 17.51 6.49 22 12 22C17.51 22 22 17.51 22 12C22 6.49 17.51 2 12 2ZM16 12.75H12.75V16C12.75 16.41 12.41 16.75 12 16.75C11.59 16.75 11.25 16.41 11.25 16V12.75H8C7.59 12.75 7.25 12.41 7.25 12C7.25 11.59 7.59 11.25 8 11.25H11.25V8C11.25 7.59 11.59 7.25 12 7.25C12.41 7.25 12.75 7.59 12.75 8V11.25H16C16.41 11.25 16.75 11.59 16.75 12C16.75 12.41 16.41 12.75 16 12.75Z"
        fill="currentColor"
      />
    </Svg>
  );
};

const AddCircleBroken = ({ size = 24, ...props }: IconProps) => {
  delete props.alterOpacity;

  return (
    <Svg width={size} height={size} viewBox="0 0 24 24" fill="none" {...props}>
      <Path d="M12 16V8" stroke="currentColor" />
      <Path d="M14.9902 12H16.0002" stroke="currentColor" />
      <Path d="M8 12H11.81" stroke="currentColor" />
      <Path d="M12 16V8" stroke="currentColor" />
      <Path
        d="M4 6C2.75 7.67 2 9.75 2 12C2 17.52 6.48 22 12 22C17.52 22 22 17.52 22 12C22 6.48 17.52 2 12 2C10.57 2 9.2 2.3 7.97 2.85"
        stroke="currentColor"
      />
    </Svg>
  );
};

const AddCircleBulk = ({ size = 24, alterOpacity, ...props }: IconProps) => {
  return (
    <Svg width={size} height={size} viewBox="0 0 24 24" fill="none" {...props}>
      <Path
        opacity={alterOpacity}
        d="M12 22C17.5228 22 22 17.5228 22 12C22 6.47715 17.5228 2 12 2C6.47715 2 2 6.47715 2 12C2 17.5228 6.47715 22 12 22Z"
        fill="currentColor"
      />
      <Path
        d="M16 11.25H12.75V8C12.75 7.59 12.41 7.25 12 7.25C11.59 7.25 11.25 7.59 11.25 8V11.25H8C7.59 11.25 7.25 11.59 7.25 12C7.25 12.41 7.59 12.75 8 12.75H11.25V16C11.25 16.41 11.59 16.75 12 16.75C12.41 16.75 12.75 16.41 12.75 16V12.75H16C16.41 12.75 16.75 12.41 16.75 12C16.75 11.59 16.41 11.25 16 11.25Z"
        fill="currentColor"
      />
    </Svg>
  );
};

const AddCircleLinear = ({ size = 24, ...props }: IconProps) => {
  delete props.alterOpacity;

  return (
    <Svg width={size} height={size} viewBox="0 0 24 24" fill="none" {...props}>
      <Path
        d="M12 22C17.5 22 22 17.5 22 12C22 6.5 17.5 2 12 2C6.5 2 2 6.5 2 12C2 17.5 6.5 22 12 22Z"
        stroke="currentColor"
      />
      <Path d="M8 12H16" stroke="currentColor" />
      <Path d="M12 16V8" stroke="currentColor" />
    </Svg>
  );
};

const AddCircleOutline = ({ size = 24, ...props }: IconProps) => {
  delete props.alterOpacity;

  return (
    <Svg width={size} height={size} viewBox="0 0 24 24" fill="none" {...props}>
      <Path
        d="M12 22.75C6.07 22.75 1.25 17.93 1.25 12C1.25 6.07 6.07 1.25 12 1.25C17.93 1.25 22.75 6.07 22.75 12C22.75 17.93 17.93 22.75 12 22.75ZM12 2.75C6.9 2.75 2.75 6.9 2.75 12C2.75 17.1 6.9 21.25 12 21.25C17.1 21.25 21.25 17.1 21.25 12C21.25 6.9 17.1 2.75 12 2.75Z"
        fill="currentColor"
      />
      <Path
        d="M16 12.75H8C7.59 12.75 7.25 12.41 7.25 12C7.25 11.59 7.59 11.25 8 11.25H16C16.41 11.25 16.75 11.59 16.75 12C16.75 12.41 16.41 12.75 16 12.75Z"
        fill="currentColor"
      />
      <Path
        d="M12 16.75C11.59 16.75 11.25 16.41 11.25 16V8C11.25 7.59 11.59 7.25 12 7.25C12.41 7.25 12.75 7.59 12.75 8V16C12.75 16.41 12.41 16.75 12 16.75Z"
        fill="currentColor"
      />
    </Svg>
  );
};

const AddCircleTwotone = ({ size = 24, alterOpacity, ...props }: IconProps) => {
  return (
    <Svg width={size} height={size} viewBox="0 0 24 24" fill="none" {...props}>
      <Path
        d="M12 22C17.5 22 22 17.5 22 12C22 6.5 17.5 2 12 2C6.5 2 2 6.5 2 12C2 17.5 6.5 22 12 22Z"
        stroke="currentColor"
      />
      <G opacity={alterOpacity}>
        <Path d="M8 12H16" stroke="currentColor" />
        <Path d="M12 16V8" stroke="currentColor" />
      </G>
    </Svg>
  );
};

const variants: IconVariants = {
  bold: AddCircleBold,
  broken: AddCircleBroken,
  bulk: AddCircleBulk,
  linear: AddCircleLinear,
  outline: AddCircleOutline,
  twotone: AddCircleTwotone,
};

export const AddCircleIcon = ({
  variant = defaultVariant,
  className,
  ...props
}: IconProps) => {
  const textClasses = useContext(TextClassContext);
  const Component = variants[variant];
  return (
    <Component
      {...defaultOptions}
      className={cn(textClasses, className)}
      {...props}
    />
  );
};

export default AddCircleIcon;
