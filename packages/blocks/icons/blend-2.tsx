import { useContext } from "react";
import { Path, Svg } from "unative/lib/svg";
import { cn } from "unative/lib/utils";
import { TextClassContext } from "unative/ui/text";

import type { IconProps, IconVariants } from "../types/icons-type";

const Blend2Bold = ({ size = 24, className, ...props }: IconProps) => {
  return (
    <Svg
      width={size}
      height={size}
      viewBox="0 0 24 24"
      fill="none"
      className={cn("text-foreground", className)}
      {...props}
    >
      <Path
        d="M15.9007 8.07C15.4507 4.64 12.5207 2 8.9707 2C5.1007 2 1.9707 5.13 1.9707 9C1.9707 12.55 4.6107 15.48 8.0407 15.93C8.3407 15.98 8.6507 16 8.9707 16C9.08708 16 9.20194 15.997 9.31589 15.991C9.53626 15.9793 9.62675 15.716 9.4707 15.56C9.1807 15.26 9.1807 14.79 9.4707 14.5C9.7607 14.2 10.2407 14.2 10.5307 14.5L11.3102 15.2795C11.4503 15.4196 11.6606 15.4653 11.8416 15.3847C12.2241 15.2145 12.5859 15.0103 12.9271 14.7743C13.1713 14.6053 13.1868 14.2581 12.9756 14.0493L12.4707 13.55C12.1807 13.26 12.1707 12.79 12.4707 12.49C12.7607 12.2 13.2307 12.19 13.5307 12.48L14.0393 12.9829C14.2491 13.1904 14.5936 13.1728 14.7616 12.9302C14.9924 12.5969 15.1932 12.2431 15.3582 11.869C15.4376 11.689 15.3913 11.4806 15.2522 11.3415L14.4707 10.56C14.1807 10.26 14.1807 9.79 14.4707 9.5C14.7607 9.2 15.2407 9.2 15.5307 9.5C15.6867 9.65604 15.95 9.56556 15.9617 9.34519C15.9677 9.23124 15.9707 9.11637 15.9707 9C15.9707 8.68 15.9507 8.37 15.9007 8.07Z"
        fill="currentColor"
      />
      <Path
        d="M21.9698 14.9991C21.9698 18.8691 18.8398 21.9991 14.9698 21.9991C12.0396 21.9991 9.52548 20.1991 8.49135 17.6319C8.36614 17.321 8.63465 16.9991 8.96977 16.9991C9.31977 16.9991 9.64977 16.9791 9.98977 16.9291L10.4948 16.8617C10.6492 16.8411 10.8044 16.8937 10.9145 17.0038L11.4398 17.5291C11.5898 17.6791 11.7798 17.7491 11.9698 17.7491C12.1698 17.7491 12.3598 17.6791 12.4998 17.5291C12.7998 17.2391 12.7998 16.7591 12.4998 16.4691C12.4034 16.3727 12.4337 16.2094 12.5556 16.1486C12.9546 15.9498 13.3634 15.6963 13.7314 15.4229C13.9392 15.2685 14.2309 15.2784 14.4151 15.4603L14.8998 15.9391C15.0498 16.0791 15.2398 16.1591 15.4298 16.1591C15.6198 16.1591 15.8198 16.0791 15.9598 15.9291C16.2498 15.6391 16.2498 15.1591 15.9498 14.8691L15.4717 14.397C15.2889 14.2165 15.2738 13.9276 15.4251 13.7201C15.6885 13.3588 15.9359 12.9657 16.1326 12.5808C16.1918 12.4649 16.3477 12.437 16.4398 12.5291C16.5898 12.6791 16.7798 12.7491 16.9698 12.7491C17.1698 12.7491 17.3598 12.6791 17.4998 12.5291C17.7998 12.2391 17.7998 11.7591 17.4998 11.4691L16.9745 10.9438C16.8644 10.8337 16.8118 10.6785 16.8324 10.5241L16.8998 10.0191C16.9498 9.67906 16.9698 9.34906 16.9698 8.99906C16.9698 8.6641 17.2917 8.39584 17.6023 8.52135C20.1621 9.55592 21.9698 12.0696 21.9698 14.9991Z"
        fill="currentColor"
      />
    </Svg>
  );
};

const Blend2Broken = ({ size = 24, className, ...props }: IconProps) => {
  return (
    <Svg
      width={size}
      height={size}
      viewBox="0 0 24 24"
      fill="none"
      className={cn("text-foreground", className)}
      {...props}
    >
      <Path
        d="M8.9707 2C12.5207 2 15.4507 4.64 15.9007 8.07C15.9507 8.37 15.9707 8.68 15.9707 9C15.9707 12.87 12.8407 16 8.9707 16C8.6507 16 8.3407 15.98 8.0407 15.93C4.6107 15.48 1.9707 12.55 1.9707 9C1.9707 7.07 2.7507 5.32 4.0207 4.05"
        stroke="currentColor"
        strokeWidth="1.5"
        strokeLinecap="round"
        strokeLinejoin="round"
      />
      <Path
        d="M21.9691 15.0001C21.9691 18.8701 18.8391 22.0001 14.9691 22.0001C11.4191 22.0001 8.48906 19.3601 8.03906 15.9301C8.33906 15.9801 8.64906 16.0001 8.96906 16.0001C12.8391 16.0001 15.9691 12.8701 15.9691 9.00007C15.9691 8.68007 15.9491 8.37007 15.8991 8.07007C19.3291 8.52007 21.9691 11.4501 21.9691 15.0001Z"
        stroke="currentColor"
        strokeWidth="1.5"
        strokeLinecap="round"
        strokeLinejoin="round"
      />
      <Path
        d="M14.1191 14.1201L15.4291 15.4101"
        stroke="currentColor"
        strokeWidth="1.5"
        strokeLinecap="round"
        strokeLinejoin="round"
      />
      <Path
        d="M16.9707 12L15.9707 11"
        stroke="currentColor"
        strokeWidth="1.5"
        strokeLinecap="round"
        strokeLinejoin="round"
      />
      <Path
        d="M11.9707 17L10.9707 16"
        stroke="currentColor"
        strokeWidth="1.5"
        strokeLinecap="round"
        strokeLinejoin="round"
      />
    </Svg>
  );
};

const Blend2Bulk = ({ size = 24, className, ...props }: IconProps) => {
  return (
    <Svg
      width={size}
      height={size}
      viewBox="0 0 24 24"
      fill="none"
      className={cn("text-foreground", className)}
      {...props}
    >
      <Path
        d="M12.4996 17.5298C12.3596 17.6798 12.1696 17.7498 11.9696 17.7498C11.7796 17.7498 11.5896 17.6798 11.4396 17.5298L9.84961 15.9398C10.4396 15.8698 11.0096 15.7298 11.5396 15.5098L12.4996 16.4698C12.7996 16.7598 12.7996 17.2398 12.4996 17.5298Z"
        fill="currentColor"
      />
      <Path
        d="M15.9591 15.9301C15.8191 16.0801 15.6191 16.1601 15.4291 16.1601C15.2391 16.1601 15.0491 16.0801 14.8991 15.9401L13.3691 14.4401C13.7591 14.1301 14.1191 13.7701 14.4291 13.3701L15.9491 14.8701C16.2491 15.1601 16.2491 15.6401 15.9591 15.9301Z"
        fill="currentColor"
      />
      <Path
        d="M17.5005 12.5299C17.3605 12.6799 17.1705 12.7499 16.9705 12.7499C16.7805 12.7499 16.5905 12.6799 16.4405 12.5299L15.4805 11.5699C15.7005 11.0399 15.8405 10.4699 15.9105 9.87988L17.5005 11.4699C17.8005 11.7599 17.8005 12.2399 17.5005 12.5299Z"
        fill="currentColor"
      />
      <Path
        d="M15.9007 8.07C15.4507 4.64 12.5207 2 8.9707 2C5.1007 2 1.9707 5.13 1.9707 9C1.9707 12.55 4.6107 15.48 8.0407 15.93C8.3407 15.98 8.6507 16 8.9707 16C9.2707 16 9.5607 15.98 9.8507 15.94L9.4707 15.56C9.1807 15.26 9.1807 14.79 9.4707 14.5C9.7607 14.2 10.2407 14.2 10.5307 14.5L11.5407 15.51C12.2107 15.25 12.8207 14.89 13.3707 14.44L12.4707 13.55C12.1807 13.26 12.1707 12.79 12.4707 12.49C12.7607 12.2 13.2307 12.19 13.5307 12.48L14.4307 13.37C14.8707 12.83 15.2307 12.23 15.4807 11.57L14.4707 10.56C14.1807 10.26 14.1807 9.79 14.4707 9.5C14.7607 9.2 15.2407 9.2 15.5307 9.5L15.9107 9.88C15.9507 9.59 15.9707 9.3 15.9707 9C15.9707 8.68 15.9507 8.37 15.9007 8.07Z"
        fill="currentColor"
      />
      <Path
        opacity="0.4"
        d="M21.9691 14.9998C21.9691 18.8698 18.8391 21.9998 14.9691 21.9998C11.4191 21.9998 8.48906 19.3598 8.03906 15.9298C8.33906 15.9798 8.64906 15.9998 8.96906 15.9998C9.26906 15.9998 9.55906 15.9798 9.84906 15.9398L11.4391 17.5298C11.5891 17.6798 11.7791 17.7498 11.9691 17.7498C12.1691 17.7498 12.3591 17.6798 12.4991 17.5298C12.7991 17.2398 12.7991 16.7598 12.4991 16.4698L11.5391 15.5098C12.2091 15.2498 12.8191 14.8898 13.3691 14.4398L14.8991 15.9398C15.0491 16.0798 15.2391 16.1598 15.4291 16.1598C15.6191 16.1598 15.8191 16.0798 15.9591 15.9298C16.2491 15.6398 16.2491 15.1598 15.9491 14.8698L14.4291 13.3698C14.8691 12.8298 15.2291 12.2298 15.4791 11.5698L16.4391 12.5298C16.5891 12.6798 16.7791 12.7498 16.9691 12.7498C17.1691 12.7498 17.3591 12.6798 17.4991 12.5298C17.7991 12.2398 17.7991 11.7598 17.4991 11.4698L15.9091 9.87982C15.9491 9.58982 15.9691 9.29982 15.9691 8.99982C15.9691 8.67982 15.9491 8.36982 15.8991 8.06982C19.3291 8.51982 21.9691 11.4498 21.9691 14.9998Z"
        fill="currentColor"
      />
    </Svg>
  );
};

const Blend2Linear = ({ size = 24, className, ...props }: IconProps) => {
  return (
    <Svg
      width={size}
      height={size}
      viewBox="0 0 24 24"
      fill="none"
      className={cn("text-foreground", className)}
      {...props}
    >
      <Path
        d="M14.97 8C14.97 11.87 11.84 15 7.97 15C7.65 15 7.34 14.98 7.04 14.93C3.61 14.48 0.970001 11.55 0.970001 8C0.970001 4.13 4.1 1 7.97 1C11.52 1 14.45 3.64 14.9 7.07C14.95 7.37 14.97 7.68 14.97 8Z"
        stroke="currentColor"
        strokeWidth="1.5"
        strokeLinecap="round"
        strokeLinejoin="round"
      />
      <Path
        d="M20.97 14.0001C20.97 17.8701 17.84 21.0001 13.97 21.0001C10.42 21.0001 7.49 18.3601 7.04 14.9301C7.34 14.9801 7.65 15.0001 7.97 15.0001C11.84 15.0001 14.97 11.8701 14.97 8.00007C14.97 7.68007 14.95 7.37007 14.9 7.07007C18.33 7.52007 20.97 10.4501 20.97 14.0001Z"
        stroke="currentColor"
        strokeWidth="1.5"
        strokeLinecap="round"
        strokeLinejoin="round"
      />
      <Path
        d="M13.12 13.1201L14.43 14.4101"
        stroke="currentColor"
        strokeWidth="1.5"
        strokeLinecap="round"
        strokeLinejoin="round"
      />
      <Path
        d="M15.97 11L14.97 10"
        stroke="currentColor"
        strokeWidth="1.5"
        strokeLinecap="round"
        strokeLinejoin="round"
      />
      <Path
        d="M10.97 16L9.97 15"
        stroke="currentColor"
        strokeWidth="1.5"
        strokeLinecap="round"
        strokeLinejoin="round"
      />
    </Svg>
  );
};

const Blend2Outline = ({ size = 24, className, ...props }: IconProps) => {
  return (
    <Svg
      width={size}
      height={size}
      viewBox="0 0 24 24"
      fill="none"
      className={cn("text-foreground", className)}
      {...props}
    >
      <Path
        d="M8.9707 16.75C8.5807 16.75 8.2407 16.72 7.9207 16.67C4.1207 16.17 1.2207 12.87 1.2207 9C1.2207 4.73 4.7007 1.25 8.9707 1.25C12.8407 1.25 16.1407 4.14 16.6407 7.97C16.6907 8.27 16.7207 8.61 16.7207 9C16.7207 13.27 13.2507 16.75 8.9707 16.75ZM8.9707 2.75C5.5207 2.75 2.7207 5.55 2.7207 9C2.7207 12.12 5.0507 14.78 8.1407 15.19C8.4007 15.23 8.6707 15.25 8.9707 15.25C12.4207 15.25 15.2207 12.45 15.2207 9C15.2207 8.7 15.2007 8.43 15.1607 8.19C14.7507 5.08 12.1007 2.75 8.9707 2.75Z"
        fill="currentColor"
      />
      <Path
        d="M14.9692 22.7502C11.0992 22.7502 7.79916 19.8602 7.29916 16.0302C7.26916 15.7902 7.34916 15.5602 7.51916 15.3902C7.68916 15.2202 7.93916 15.1502 8.16916 15.1902C8.40916 15.2302 8.66916 15.2502 8.96916 15.2502C12.4192 15.2502 15.2192 12.4502 15.2192 9.00022C15.2192 8.70022 15.1992 8.43022 15.1592 8.19022C15.1192 7.96022 15.1992 7.72022 15.3592 7.55022C15.5192 7.38022 15.7492 7.29022 15.9992 7.33022C19.8292 7.83022 22.7192 11.1302 22.7192 15.0002C22.7192 19.2702 19.2492 22.7502 14.9692 22.7502ZM8.97916 16.7502C9.73916 19.3702 12.1692 21.2502 14.9692 21.2502C18.4192 21.2502 21.2192 18.4502 21.2192 15.0002C21.2192 12.2002 19.3392 9.76022 16.7192 9.01022C16.7192 13.2702 13.2492 16.7502 8.97916 16.7502Z"
        fill="currentColor"
      />
      <Path
        d="M15.4295 16.1598C15.2395 16.1598 15.0495 16.0898 14.8995 15.9498L13.5995 14.6698C13.3095 14.3798 13.2995 13.8998 13.5895 13.6098C13.8795 13.3198 14.3495 13.3098 14.6495 13.5998L15.9495 14.8798C16.2395 15.1698 16.2495 15.6498 15.9595 15.9398C15.8195 16.0798 15.6195 16.1598 15.4295 16.1598Z"
        fill="currentColor"
      />
      <Path
        d="M16.9702 12.7499C16.7802 12.7499 16.5902 12.6799 16.4402 12.5299L15.4402 11.5299C15.1502 11.2399 15.1502 10.7599 15.4402 10.4699C15.7302 10.1799 16.2102 10.1799 16.5002 10.4699L17.5002 11.4699C17.7902 11.7599 17.7902 12.2399 17.5002 12.5299C17.3602 12.6799 17.1702 12.7499 16.9702 12.7499Z"
        fill="currentColor"
      />
      <Path
        d="M11.9702 17.7499C11.7802 17.7499 11.5902 17.6799 11.4402 17.5299L10.4402 16.5299C10.1502 16.2399 10.1502 15.7599 10.4402 15.4699C10.7302 15.1799 11.2102 15.1799 11.5002 15.4699L12.5002 16.4699C12.7902 16.7599 12.7902 17.2399 12.5002 17.5299C12.3602 17.6799 12.1702 17.7499 11.9702 17.7499Z"
        fill="currentColor"
      />
    </Svg>
  );
};

const Blend2Twotone = ({ size = 24, className, ...props }: IconProps) => {
  return (
    <Svg
      width={size}
      height={size}
      viewBox="0 0 24 24"
      fill="none"
      className={cn("text-foreground", className)}
      {...props}
    >
      <Path
        d="M14.9707 8C14.9707 11.87 11.8407 15 7.9707 15C7.6507 15 7.3407 14.98 7.0407 14.93C3.6107 14.48 0.970703 11.55 0.970703 8C0.970703 4.13 4.1007 1 7.9707 1C11.5207 1 14.4507 3.64 14.9007 7.07C14.9507 7.37 14.9707 7.68 14.9707 8Z"
        stroke="currentColor"
        strokeWidth="1.5"
        strokeLinecap="round"
        strokeLinejoin="round"
      />
      <Path
        opacity="0.4"
        d="M20.971 13.9998C20.971 17.8698 17.841 20.9998 13.971 20.9998C10.421 20.9998 7.49102 18.3598 7.04102 14.9298C7.34102 14.9798 7.65102 14.9998 7.97102 14.9998C11.841 14.9998 14.971 11.8698 14.971 7.99982C14.971 7.67982 14.951 7.36982 14.901 7.06982C18.331 7.51982 20.971 10.4498 20.971 13.9998Z"
        stroke="currentColor"
        strokeWidth="1.5"
        strokeLinecap="round"
        strokeLinejoin="round"
      />
      <Path
        opacity="0.4"
        d="M13.1211 13.1201L14.4311 14.4101"
        stroke="currentColor"
        strokeWidth="1.5"
        strokeLinecap="round"
        strokeLinejoin="round"
      />
      <Path
        opacity="0.4"
        d="M15.9707 11L14.9707 10"
        stroke="currentColor"
        strokeWidth="1.5"
        strokeLinecap="round"
        strokeLinejoin="round"
      />
      <Path
        opacity="0.4"
        d="M10.9707 16L9.9707 15"
        stroke="currentColor"
        strokeWidth="1.5"
        strokeLinecap="round"
        strokeLinejoin="round"
      />
    </Svg>
  );
};

const variants: IconVariants = {
  bold: Blend2Bold,
  broken: Blend2Broken,
  bulk: Blend2Bulk,
  linear: Blend2Linear,
  outline: Blend2Outline,
  twotone: Blend2Twotone,
};

export const Blend2Icon = ({
  variant = "linear",
  className,
  ...props
}: IconProps) => {
  const textClasses = useContext(TextClassContext);
  const Component = variants[variant];
  return <Component className={cn(textClasses, className)} {...props} />;
};
